"use client"

import { useState } from "react"
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card"
import { Button } from "@/components/ui/button"
import { Input } from "@/components/ui/input"
import { Collapsible, CollapsibleContent, CollapsibleTrigger } from "@/components/ui/collapsible"
import { Trash2, ChevronRight, FileText, Plus, ExternalLink, Ban, Eye, Info, Import } from "lucide-react"
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
  TooltipTrigger,
} from "@/components/ui/tooltip"
import { RobotsImportModal } from "./import-modal/robot-modal"

interface ContentZone {
  id: string
  title: string
  paths: string[]
  pathCount: number
  rules?: number
}

export function ContentZones() {
  const [zones] = useState<ContentZone[]>([
    {
      id: "1",
      title: "Imported paths",
      paths: ["/*tid_external_content_proxy/", "/.ei=", "/.linkId="],
      pathCount: 22,
    },
    {
      id: "2",
      title: "Imported paths",
      paths: ["/finanzen/versichern-und-vorsorgen/"],
      pathCount: 1,
      rules: 8,
    },
  ])

  const [expanded, setExpanded] = useState<string>("")
  const [importModalOpen, setImportModalOpen] = useState(false)

  return (
    <>
      <Card className="border-border/50">
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle className="text-base font-medium flex items-center gap-2">
            <FileText className="w-4 h-4 text-muted-foreground" />
            Content zones
            <TooltipProvider>
              <Tooltip delayDuration={0}>
                <TooltipTrigger asChild>
                  <Info className="w-4 h-4 text-muted-foreground/50 cursor-help" />
                </TooltipTrigger>
                <TooltipContent className="max-w-[280px] p-3">
                  <p className="text-xs leading-relaxed">
                    Zones allow you to group multiple platform paths and apply unified firewall policies across them. Changes made here override global domain settings for these specific paths.
                  </p>
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
          </CardTitle>
          <div className="flex items-center gap-2">
            <div className="relative">
              <Input
                placeholder="Search..."
                className="w-32 h-8 text-xs bg-muted/30 border-border/50"
              />
            </div>
            <Button
              variant="ghost"
              size="sm"
              className="text-red-500 hover:text-red-400 hover:bg-red-500/10 h-8 px-2"
            >
              <Trash2 className="w-3.5 h-3.5" />
            </Button>
          </div>
        </CardHeader>
        <CardContent className="space-y-2">
          {zones.map((zone) => (
            <Collapsible 
              key={zone.id} 
              open={expanded === zone.id} 
              onOpenChange={() => setExpanded(expanded === zone.id ? "" : zone.id)}
            >
                <div className="border border-border/50 rounded-lg overflow-hidden relative">
                  <CollapsibleTrigger asChild>
                    <div className="flex items-center justify-between p-3 hover:bg-muted/30 transition-colors cursor-pointer">
                      <div className="flex items-center gap-3">
                        <ChevronRight className={`w-4 h-4 text-muted-foreground transition-transform ${expanded === zone.id ? "rotate-90" : ""}`} />
                        <div className="text-left">
                          <p className="font-medium text-sm">
                            {zone.title} ({zone.pathCount})
                          </p>
                          <p className="text-xs text-muted-foreground mt-0.5">
                            {zone.paths.slice(0, 2).join(", ")}
                            {zone.pathCount > 2 && ` ... and ${zone.pathCount - 2} more`}
                          </p>
                        </div>
                      </div>
                      <div className="w-32" /> 
                    </div>
                  </CollapsibleTrigger>
                  
                  <div className="absolute right-3 top-3 flex items-center gap-2 z-10">
                    <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground" onClick={(e) => e.stopPropagation()}>
                      <ExternalLink className="w-3.5 h-3.5" />
                    </Button>
                    <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground" onClick={(e) => e.stopPropagation()}>
                      <Ban className="w-3.5 h-3.5" />
                    </Button>
                    <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground" onClick={(e) => e.stopPropagation()}>
                      <Eye className="w-3.5 h-3.5" />
                    </Button>
                    {zone.rules && (
                      <span className="text-xs text-muted-foreground px-2">{zone.rules} rules</span>
                    )}
                    <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground hover:text-red-500" onClick={(e) => e.stopPropagation()}>
                      <Trash2 className="w-3.5 h-3.5" />
                    </Button>
                  </div>

                <CollapsibleContent>
                  <div className="px-4 pb-4 border-t border-border/50 pt-3 space-y-2">
                    {zone.paths.map((path, idx) => (
                      <div key={idx} className="flex items-center justify-between p-2.5 bg-muted/30 rounded border border-border/30">
                        <code className="text-xs text-muted-foreground font-mono">{path}</code>
                      </div>
                    ))}
                  </div>
                </CollapsibleContent>
              </div>
            </Collapsible>
          ))}

          <div className="flex gap-2 pt-2">
            <Button 
              variant="outline" 
              className="flex-1 gap-2 text-xs h-9 border-dashed"
              onClick={() => {}}
            >
              <Plus className="w-3.5 h-3.5" />
              Add zone
            </Button>
            <Button 
              variant="outline"
              className="gap-2 text-xs h-9 bg-blue-500/5 text-blue-600 border-blue-500/20 hover:bg-blue-500/10 transition-colors"
              onClick={() => setImportModalOpen(true)}
            >
              <Import className="w-3.5 h-3.5" />
              Import robots.txt
            </Button>
          </div>
        </CardContent>
      </Card>

      <RobotsImportModal 
        open={importModalOpen} 
        onOpenChange={setImportModalOpen}
      />
    </>
  )
}

